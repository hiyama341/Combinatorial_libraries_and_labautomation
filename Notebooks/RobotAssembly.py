...
def AddPrimersToPlates(unikke_amplicons, start = 0, end = 96):
    '''This function adds primers to virtual plates from amplicons'''
    
    # Getting keys for wells
    well_keys = well_keys_96()
    
    # Initializing plate with platetype and name
    ...
    # Calculate the needed volume and adding a little extra to be sure there is enough mixture
    volume_needed = len(unikke_amplicons)+1
    
    # Going through the amplicons to and creating virtual plates
    ############## By changing the slicing the user can make different plates i.e 97-192 or 193-284 etc. #"
    for i in range(0, len(unikke_amplicons[start:end])):
        #Forward primer plate
        source_plate1[well_keys[i]].add_content({unikke_amplicons[i].forward_primer.id:1},volume=5)
        # Reverse primer plate
        source_plate2[well_keys[i]].add_content({unikke_amplicons[i].reverse_primer.id:1},volume=5 )
        # Template plate
        source_plate3[well_keys[i]].add_content({unikke_amplicons[i].name:1},volume=5)
        
        # PCRMIX and H2O plate 
...
def PickListen(unikke_amplicons, source_plate1, source_plate2, source_plate3, source_plate4, start = 0, end = 96):
    """
    This function makes a picklist from amplicons and 4 virtual plates and adds them to a destination plate (plate 5). 
    The 4 plates are made as follows: 
    
    source_plate1  # Forward primers
    source_plate2  # Reverse primers
    source_plate3  # Templates 
    source_plate4  # PCR mix and H20
    
    """

    # Initalizing destination plate and picklistObject: 
    ...
    for i in range(0, len(unikke_amplicons[start:end])):
        # F_primers
        transfer_forward_primers = lab.Transfer(source_plate1.wells[well_keys[i]], destination_plate.wells[well_keys[i]], 1 )
        picklist.add_transfer(transfer = transfer_forward_primers)

        # R_primers
        transfer_reverse_primers = lab.Transfer(source_plate2.wells[well_keys[i]], destination_plate.wells[well_keys[i]], 1)
        picklist.add_transfer(transfer = transfer_reverse_primers)

        # Templates
        transfer_templates = lab.Transfer(source_plate3.wells[well_keys[i]], destination_plate.wells[well_keys[i]], 1 )
        picklist.add_transfer(transfer = transfer_templates)
        
        #PCR mix
        transfer_PCR_mix = lab.Transfer(source_plate4.wells['A1'],destination_plate.wells[well_keys[i]], 10 )
        picklist.add_transfer(transfer = transfer_PCR_mix)
        
        # H2O
        H20 = lab.Transfer(source_plate4.wells['A2'],destination_plate.wells[well_keys[i]], 7 )
        picklist.add_transfer(transfer = H20)

#     # Find out which wells have primers+templates in them
#     used_wells = []
#     for well in destination_plate.iter_wells():
#         if well.volume > 0:
#             used_wells.append(well.name)

#     # Then we add the PCR AND H20 in the end        
#     for i in range(0, len(used_wells)): 
#     #     PCR_mix
#         transfer_PCR_mix = lab.Transfer(source_plate4.wells['A1'],destination_plate.wells[used_wells[i]], 10 )
#         picklist.add_transfer(transfer = transfer_PCR_mix)

#     for j in range(0, len(used_wells)): 
#         # H2O
#         H20 = lab.Transfer(source_plate4.wells['A2'],destination_plate.wells[used_wells[j]], 7 )
#         picklist.add_transfer(transfer = H20)

    return picklist

def MakeVirtualPlatesFromDF(f_primers:list, r_primers:list, templates:list, Dataframe_with_PCR_contents): 
    '''
...
def PicklistFromPlates(F_primer_plate, R_primer_plate, Templates_plate, MM_H20_plate, PCR_dataframe):
...
class RobotAssembly:
    ...
    def __init__(self, DesignAssembly_object = None, Pandas_dataframe_PCR = None, F_primers:list=None, R_primers:list = None, Templates:list = None):
        
        ###  1.INITIALIZING ##
        
        ######################## Naive assembly ############################
        if DesignAssembly_object is not None and Pandas_dataframe_PCR is None and F_primers is None and R_primers is None and Templates is None: 
            # Making virtual plates
            self.source_plateF_primers, self.source_plateR_primers, self.source_plateTemplates, self.source_platePCRmix = AddPrimersToPlates(DesignAssembly_object.unique_amplicons)
    
            # Generating Picklist from virtual plates
            self.picklist = PickListen(DesignAssembly_object.unique_amplicons, self.source_plateF_primers, self.source_plateR_primers, self.source_plateTemplates, self.source_platePCRmix)
        ...
        if Pandas_dataframe_PCR is not None and F_primers is not None and R_primers is not None and Templates is not None:
            ...
            #Virtual plates
            self.source_plateF_primers, self.source_plateR_primers, self.source_plateTemplates, self.source_platePCRmix = MakeVirtualPlatesFromDF( self.forward_primers, self.reverse_primers, self.templates, self.pandas_PCR)
            
            # Generating Picklist from virtual plates
            self.picklist = PicklistFromPlates(self.source_plateF_primers, self.source_plateR_primers, self.source_plateTemplates, self.source_platePCRmix, self.pandas_PCR)
            
            
            
        ######################## ERROR input messages ######################## Some std error messages
        if DesignAssembly_object is not None and F_primers is not None: 
            print('There seems to be a mistake with the input. Try agian. Remember you can only use amplicons or Dataframes for robot instructions')
        if DesignAssembly_object is not None and R_primers is not None:
            print('There seems to be a mistake with the input. Try agian. Remember you can only use amplicons or Dataframes for robot instructions')
        if DesignAssembly_object is not None and Templates is not None: 
            print('There seems to be a mistake with the input. Try agian. Remember you can only use amplicons or Dataframes for robot instructions')
        if DesignAssembly_object is not None and Pandas_dataframe_PCR is not None: 
            print('There seems to be a mistake with the input. Try agian. Remember you can only use amplicons or Dataframes for robot instructions')
