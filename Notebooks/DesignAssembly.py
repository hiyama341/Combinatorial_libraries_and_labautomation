...
def primer_TM_NEB(primer1: str):

    #TMCalculator: find melting temperature and annealing temperature
    url = 'https://tmapi.neb.com/tm/batch'

    #Define if it is a primer pair or single primer
    seqpairs = [[primer1]]
    ...
    #Print TM and maybe Ta + possible error messages
    if r['success']:
        for row in r['data']:
            return row['tm1']
    else:
...
def SimpleAmpliconMaker(list_of_seqs:list, list_of_names:list): 
    ...
    for i in range(0,len(list_of_seqs)):
        for j in range(0, len(list_of_seqs[i])): 
            # Append Amplicons
            amplicons = primer_design(list_of_seqs[i][j],tm_func=_tm_default,  target_tm = 56.0, limit = 13)   ############## Can add NEB Calculator here: primer_TM ################# _tm_default i.e tm_func = _tm_default,
            ...
            melting_temps = (_tm_default(amplicons.forward_primer.seq),_tm_default(amplicons.reverse_primer.seq))
...
def GetPrimers(List_of_assemblies:list, combinatorial_list_of_names:list, combinatorial_list_of_primer_tm:list): 
    ...
    for i in range(0,len(List_of_assemblies)):
        for j in range(0, len(List_of_assemblies[i])):
            ...
            if j == 0:   # START OF THE ASSEMBLY
                List_of_assemblies[i][j].forward_primer.description='Anneals to '+str(List_of_assemblies[i][j].name) 
                List_of_assemblies[i][j].reverse_primer.description='Anneals to '+ str(List_of_assemblies[i][j].name)+', overlaps to '+ str(List_of_assemblies[i][j+1].name)                                                                                                                             
            if j > 0 and j < len(List_of_assemblies[i])-1: #      # THE rest:
                List_of_assemblies[i][j].forward_primer.description='Anneals to '+str(List_of_assemblies[i][j].name+', overlaps to '+ str(List_of_assemblies[i][j-1].name))   
                List_of_assemblies[i][j].reverse_primer.description='Anneals to '+str(List_of_assemblies[i][j].name+', overlaps to '+ str(List_of_assemblies[i][j+1].name)) 
            if j == len(List_of_assemblies[i])-1: # THE END OF THE ASSEMBLY
                List_of_assemblies[i][j].forward_primer.description='Anneals to '+str(List_of_assemblies[i][j].name)+', overlaps to '+ str(List_of_assemblies[i][j-1].name) 
                List_of_assemblies[i][j].reverse_primer.description='Anneals to '+ str(List_of_assemblies[i][j].name)                                                                                                                             

            #template it aneals to 
            List_of_assemblies[i][j].forward_primer.name = str(List_of_assemblies[i][j].name)
            List_of_assemblies[i][j].reverse_primer.name = str(List_of_assemblies[i][j].name) 

            # Primer tm
            List_of_assemblies[i][j].forward_primer.features =round(float(combinatorial_list_of_primer_tm[i][j][0]), 2) 
            List_of_assemblies[i][j].reverse_primer.features =round(float(combinatorial_list_of_primer_tm[i][j][1]),2)

            fwd_rev_primers = [List_of_assemblies[i][j].forward_primer, List_of_assemblies[i][j].reverse_primer]
...
def AssemblyMaker(combinatorial_list_of_amplicons:list):
    ...
    for i in range(0, len(combinatorial_list_of_amplicons)): 
        List_of_assemblies.append(assembly_fragments(combinatorial_list_of_amplicons[i], overlap=35, maxlink=40))
...
def UniquePrimers(primers:list, list_of_assemblies):    
    ...
    for i in range(len(unikke_F_primers)):
        ...
        U_f_primers = [unikke_F_primers[i].id,unikke_F_primers[i].name,unikke_F_primers[i].seq, unikke_F_primers[i].features,len(unikke_F_primers[i].seq),len(unikke_F_primers[i].seq)*1.8  ]
    ...
    for i in range(len(unikke_R_primers)): 
        ...
        U_r_primers = [unikke_R_primers[i].id,unikke_R_primers[i].name,unikke_R_primers[i].seq, unikke_R_primers[i].features,len(unikke_R_primers[i].seq),len(unikke_R_primers[i].seq)*1.8]
        unique_reverse_primers.append(U_r_primers)
        
    primer_list = unique_forward_primers +  unique_reverse_primers # cOULD CONCATONATE THEM INTO: unique_forward_primers + unique_reverse_primers
    
                ### Updating primer names and removing duplicates?
    for i in range(0, len(list_of_assemblies)):
        for j in range(0, len(list_of_assemblies[i])):
            for l in range(0, len(unikke_F_primers)):
                if list_of_assemblies[i][j].forward_primer.seq == unikke_F_primers[l].seq: 
                    list_of_assemblies[i][j].forward_primer = unikke_F_primers[l]
            for m in range(0, len(unique_reverse_primers)):
                if list_of_assemblies[i][j].reverse_primer.seq == unikke_R_primers[m].seq: 
...
class DesignAssembly:
    ...
    def __init__(self, list_of_seqs:list,list_of_names:list, pad:str, position_of_pad:int):
        ...
        self.list_of_amplicons, self.list_of_amplicon_primers, self.list_of_amplicon_primer_temps = SimpleAmpliconMaker(self.list_of_seqs, self.list_of_names)
        
        # Systematic names
        self.systematic_names = systematic_names_function(self.list_of_seqs)
        
        ### 3. COMBINATORIAL LISTS
        self.combinatorial_list_of_amplicons = CombinatorialListMaker(self.list_of_amplicons)
        self.combinatorial_list_of_names = CombinatorialListMaker(self.list_of_names)
        self.combinatorial_list_of_primer_tm = CombinatorialListMaker(self.list_of_amplicon_primer_temps)
        ...
        for i in range(0, len(self.combinatorial_list_of_amplicons)): 
            self.combinatorial_list_of_amplicons[i] = list(self.combinatorial_list_of_amplicons[i])
            
        #### 4. Adding PAD ### 
#         protospacer = Dseqrecord(self.pad)
        for i in range(0, len(self.combinatorial_list_of_amplicons)): 
            self.combinatorial_list_of_amplicons[i].insert(self.position_of_pad, self.pad)
        ...
        self.primers = GetPrimers(self.list_of_assemblies, self.combinatorial_list_of_names, self.combinatorial_list_of_primer_tm)
    ...
    def ShowContigs(self):
        ...
        for i in range(0, len(self.list_of_assemblies)):
            ...
            for j in range(0, len(self.list_of_assemblies[i])):
                print('Template: ',self.list_of_assemblies[i][j].name[0:15])#, '\t', self.primers[i][j][0].name,'\t',self.primers[i][j][0].features)
        return 
    ...
    def ShowVariantsLibDF(self): 
        ...
        combinatorial_lib_variants_df['Variant'] = np.arange(len(combinatorial_lib_variants_df))
    ...
    def primer_list_to_dataframe(self): 
        ...
        df.columns =['ID', 'Anneals to', 'Sequence', 'Annealing temperature', 'Length', 'Price(DKK)']
    ...
    def print_PCR_list(self): 
        ...
        for i in range(0,len(self.unique_amplicons)):
            print('PCR{number}'.format(number = i+1),',',self.unique_amplicons[i].name,',', self.unique_amplicons[i].forward_primer.id,',', self.unique_amplicons[i].reverse_primer.id,',', self.unique_amplicons[i].forward_primer.features,',', self.unique_amplicons[i].reverse_primer.features)
    
    def PCR_list_to_dataframe(self):
        ''' Prints PCR_list into a pandas dataframe'''
        dataframe_list = []        
        for i in range(0,len(self.unique_amplicons)):
            lst = ['PCR{number}'.format(number = i+1),self.unique_amplicons[i].name, self.unique_amplicons[i].forward_primer.id, self.unique_amplicons[i].reverse_primer.id, self.unique_amplicons[i].forward_primer.features,self.unique_amplicons[i].reverse_primer.features]
            dataframe_list.append(lst)
            
        df = pd.DataFrame(dataframe_list)
        df.columns =['PCR#', 'Template', 'forward_primer', 'reverse_primer', 'F_tm','R_tm']
    ...
    def GraphicalRepresentationOfAssemblies(self):
        ...
        '''
        graphical_representation = [self.assembly_object[x].assemble_linear()[0].figure() for x in range(0,len(self.assembly_object))]
